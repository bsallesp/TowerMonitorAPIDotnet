...
namespace TowerSimulator.Worker;

public class TowerStatusSimulatorHostedService(IServiceScopeFactory scopeFactory, CosmosClient cosmos) : BackgroundService
{
    private readonly Dictionary<long, TowerLiveStatus> _towerStates = new();
    private readonly Container _container = cosmos.GetContainer("TowerDb", "TowerLiveStatus");

    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        var nextHeartbeat = DateTimeOffset.UtcNow;
        var nextIncidents = DateTimeOffset.UtcNow;
        var nextPurge = NextUtc(TimeSpan.FromHours(3));
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            if (now >= nextPurge)
            {
                ...
                await PurgeOldTelemetryAsync(scope3.ServiceProvider, 7, stoppingToken);
                nextPurge = NextUtc(TimeSpan.FromHours(3));
            }
    ...
    static DateTimeOffset NextUtc(TimeSpan utcTime)
    {
        var now = DateTimeOffset.UtcNow;
        var today = new DateTimeOffset(now.Year, now.Month, now.Day, 0, 0, 0, TimeSpan.Zero) + utcTime;
    ...
    private async Task PurgeOldTelemetryAsync(IServiceProvider sp, int days, CancellationToken ct)
    {
        var towerRepo = sp.GetRequiredService<ITowerRepository>();
        var ids = await towerRepo.GetIdsAsync(ct);
        foreach (var towerId in ids)
            await _container.Scripts.ExecuteStoredProcedureAsync<string>("purgeOldTelemetry", new PartitionKey(towerId), new object[] { days }, cancellationToken: ct);
    }
}
